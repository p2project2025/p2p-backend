...
func (r *WithdrawlRepo) UpdateWithdrawStatus(withdrawID string, approve bool) error {
	...
	if approve {
		...
		err := userCollection.FindOne(ctx, bson.M{"_id": wd.UserId}).Decode(&user)
		if err != nil {
...
func (r *WithdrawlRepo) GetAllByUserID(userID string) ([]models.WithdrawlRes, error) {
	...
	// 1️⃣ Fetch withdrawals for the given user (no pagination)
	findOpts := options.Find().
		SetSort(bson.M{"created_at": -1})
	...
	// 2️⃣ Fetch user info
	var user models.User
	if err := userCollection.FindOne(ctx, bson.M{"_id": oid}).Decode(&user); err != nil && !errors.Is(err, mongo.ErrNoDocuments) {
		return nil, err
	}
