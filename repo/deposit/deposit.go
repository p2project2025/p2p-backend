...
func (r *DepositRepo) GetAllByUserID(userID string) ([]models.DepositRes, error) {
	...
	if userID != "" {
		...
		if err != nil {
			...
			return []models.DepositRes{}, nil // ✅ return empty slice, not error
		}
		filter["user_id"] = oid
	}
	...
	// ✅ Explicitly handle empty deposits
	if len(deposits) == 0 {
		log.Println("No deposits found for user:", userID)
		return []models.DepositRes{}, nil
	}

	// 2️⃣ Fetch user info for each deposit
	results := make([]models.DepositRes, 0, len(deposits))
